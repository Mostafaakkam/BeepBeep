...
- Explicitly scoped as v1: list/switch stores only (no store-creation UI or endpoint — none existed before this feature and none was added); product add/edit/deactivate/reactivate (soft-delete only, full ACTIVE ↔ INACTIVE cycle since the 2026-08-30 stabilization audit, images set at creation only, no image upload); order status advancement one step at a time along a fixed forward path; **no Admin Dashboard work**
...
- **Correction (2026-08-30 stabilization audit):** the claim on this line that "no DB ENUM/CHECK constrains this column" was never actually checked against the live database and was wrong. `orders.status` IS a real MySQL/MariaDB `ENUM`, and the live database's enum was still `enum('pending','confirmed','shipping','delivered','cancelled')` — missing `preparing`/`shipped` entirely, which `orderService.VALID_TRANSITIONS` (below) requires. This meant a store owner could advance an order to `confirmed` and no further; every subsequent status update was rejected by the database itself, not by application logic. Fixed via `database/migrations/004_widen_order_status_enum.sql` (additive — widens the enum to include `preparing`/`shipped`, keeps the unused legacy `shipping` value for backward compatibility). `product_variants` already has exactly the columns needed (`color, size, price, stock`) — nothing invented

**Backend Architecture:**
- No authorization middleware was modified. `requireRole`, `requireStoreOwnership`, `requireProductOwnership` (all from the prior Role System feature) are reused exactly as-is on every new endpoint; `requireProductOwnership` is mounted on a real route for the first time (`PUT /api/products/:id`, `PATCH /api/products/:id/deactivate`, and `PATCH /api/products/:id/reactivate` — added 2026-08-30, same chain)
...
- `productService.js` — `validateProductData()` (name/description length limits, category must exist via `categoryRepository.findById`, at least one variant with `price > 0` and integer `stock >= 0`), `createProduct`/`updateProduct`/`deactivateProduct`/`reactivateProduct` (the last added 2026-08-30 — `productRepository.setActive()` always accepted a boolean, but nothing called it with `true` until this fix), all `.code`-tagged errors following the existing convention
