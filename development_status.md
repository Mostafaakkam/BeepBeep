...
\*\*Last Updated:\*\* 2026-08-30 (Stabilization audit — live database verified directly, a critical order-status schema mismatch found and fixed, product reactivation gap closed)
...
\- \*\*Database Migrations:\*\* No system for version-controlled schema changes (use manual SQL updates) — confirmed again during the 2026-08-30 audit: no migration runner, migration-tracking table, or related package exists anywhere in `backend/package.json`. Four manual migration files now live in `database/migrations/`: `001\_create\_reviews\_table.sql` (creates the `reviews` table), `002\_add\_roles\_ownership\_single\_store.sql` (adds `stores.owner\_id`, `carts.store\_id`, `orders.store\_id`, all nullable, plus indexes and FKs), `003\_add\_product\_is\_active.sql` (adds `products.is\_active`, `DEFAULT 1`, plus an index — Store Owner Dashboard soft-delete), and `004\_widen\_order\_status\_enum.sql` (widens `orders.status` to include `preparing`/`shipped` — see "Order Status" below; \*\*not yet applied to the live database as of this writing\*\*). Run migrations by hand, in order, against your `beep\_beep` database; all four are purely additive and do not remove or retype any existing column. \*\*As of the 2026-08-30 audit, migrations 001–003 were confirmed applied to the live database directly via phpMyAdmin\*\* (`reviews` table exists, `users.role` includes `store\_owner`, `stores.owner\_id`/`carts.store\_id`/`orders.store\_id`/`products.is\_active` all present with their expected foreign keys and indexes) — migration 004 is the one still outstanding.

\- \*\*Order Status:\*\* The application-level flow (`pending → confirmed → preparing → shipped → delivered`, one step at a time, enforced server-side by `orderService.js`'s transition map) was correct and complete as of the Store Owner Dashboard feature. \*\*However, the 2026-08-30 stabilization audit found the LIVE database's `orders.status` column was still `enum('pending','confirmed','shipping','delivered','cancelled')` — it never had `'preparing'` or `'shipped'` added\*\*, because all prior verification for this feature ran against a disposable sandbox database, never the live one. In practice this meant a store owner could move an order from `pending` to `confirmed` and no further — every status update past that point would be rejected by MariaDB's strict-mode enum enforcement. Fixed via `database/migrations/004\_widen\_order\_status\_enum.sql` (purely additive — widens the enum to include `preparing`/`shipped`, keeps `shipping` for backward compatibility, changes nothing else). \*\*This migration must still be applied to the live database by hand\*\* (see `database/migrations/004\_widen\_order\_status\_enum.sql`'s header for the exact statement and rationale) — it was written and verified during this audit but not yet executed against the live `beep\_beep` database as of this update.
...
\- \*\*Seed Data Script:\*\* `backend/src/seeders/seed.js` populates the database with safe, fully-fake demo data and is re-runnable (it truncates and re-inserts the relevant tables). It does not touch `orders`/`order\_items`. See `backend/README.md`. \*\*Verified live 2026-08-30:\*\* 5 users (`ahmad`/`sara` = `store\_owner`, 3 = `customer`); 5 stores (`ahmad` owns 2, `sara` owns 1, the electronics store and the pending gifts store are both intentionally unowned — matches the seed script's own design, not a bug); 15 products, all `is\_active = 1`, correctly distributed only across the 4 active stores; carts each scoped to exactly one store. No inconsistency found between seed data and the current schema/roles/ownership model.
